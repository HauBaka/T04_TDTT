...
class AddMembersRequest(BaseModel):
    member_uids: list[str] = Field(
        ...
        min_length=1,
        max_length=10,
        description="Danh sách UID của các thành viên cần thêm vào conversation. Tối đa 10 thành viên mỗi lần thêm.",
    )
...
class RemoveMembersRequest(BaseModel):
    member_uids: list[str] = Field(
        ...
        min_length=1,
        max_length=10,
        description="Danh sách UID của các thành viên cần xóa khỏi conversation. Tối đa 10 thành viên mỗi lần xóa.",
    )
...
class SendMessageRequest(BaseModel):
    ...
    attachments: list[ConversationMessageAttachment] = Field(
        default_factory=list,
        min_length=0,
        max_length=5,
        description="Danh sách các tệp đính kèm cho tin nhắn. Tối đa 5 tệp đính kèm mỗi tin nhắn.",
    )
...
class ConversationMessageDocument(BaseModel):
    ...
    )  # Danh sách các tệp đính kèm (nếu có)
...
class ConversationDocument(BaseModel):
    ...
    member_uids: list[str] = Field(
        default_factory=list
    )  # Chỉ lưu UID của thành viên trong document gốc để tối ưu truy vấn, thông tin chi tiết sẽ được lấy từ sub-collection "members"
