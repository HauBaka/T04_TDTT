...
class UploadPresignRequest(
    BaseModel
):  # Request body khi client yêu cầu presign URL để upload file lên R2
...
class UploadPresignResponse(
    BaseModel
):  # Response trả về khi client yêu cầu presign URL để upload file lên R2
    upload_url: str
    file_key: str
    public_url: str
    expires_at: str
...
class UploadConfirmRequest(
    BaseModel
):  # Request body khi client thông báo đã upload file thành công lên R2 và muốn xác nhận để lưu metadata vào DB
...
class UploadConfirmResponse(
    BaseModel
):  # Response trả về khi client xác nhận việc upload file thành công
    file_key: str
    public_url: str
    size_bytes: int
    content_type: str
