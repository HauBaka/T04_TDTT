...
async def create_trip(
    trip_request: TripCreateRequest, requester=Depends(get_current_user(optional=False))
):
...
async def get_trip(trip_id: str, requester=Depends(get_current_user(optional=True))):
    ...
    return await trip_service.get_trip(
        trip_id, requester.get("uid") if requester else None
    )
...
async def update_trip(
    trip_id: str,
    trip_request: TripUpdateRequest,
...
async def delete_trip(
    trip_id: str, requester=Depends(get_current_user(optional=False))
):
    """Xóa một trip."""
...
@trip_router.post(
    "/trips/{trip_id}/members", response_model=ResponseSchema[TripResponse]
)
async def add_members_to_trip(
    trip_id: str,
    request: TripAddMembersRequest,
    requester=Depends(get_current_user(optional=False)),
):
    """Thêm nhiều thành viên vào một trip."""
    return await trip_service.add_members_to_trip(
        trip_id, requester.get("uid"), request.member_uids
    )
...
async def remove_members_from_trip(
    trip_id: str,
    request: TripRemoveMembersRequest,
    requester=Depends(get_current_user(optional=False)),
):
    """Xóa nhiều thành viên khỏi một trip."""
    return await trip_service.remove_members_from_trip(
        trip_id, requester.get("uid"), request.member_uids
    )


@trip_router.get(
    "/trips/{trip_id}/members", response_model=ResponseSchema[list[TripMemberResponse]]
)
