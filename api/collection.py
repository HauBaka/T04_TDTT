...
@collection_router.post(
    "/collections", response_model=ResponseSchema[CollectionResponse]
)
async def create_collection(
    ...
    return await collection_service.create_collection(
        requester.get("uid"), collection_request
    )


@collection_router.get(
    "/collections/{collection_id}", response_model=ResponseSchema[CollectionResponse]
)
async def get_collection(
    collection_id: str, requester=Depends(get_current_user(optional=True))
):
    """Lấy thông tin của một collection cụ thể."""
    return await collection_service.get_collection(
        collection_id, requester.get("uid") if requester else None
    )
...
async def update_collection(
    ...
    return await collection_service.update_collection(
        collection_id, requester.get("uid"), collection_request
    )
...
@collection_router.post(
    ...
    response_model=ResponseSchema[CollectionResponse],
)
...
@collection_router.get(
    "/collections/{collection_id}/places",
    response_model=ResponseSchema[list[CollectionPlaceResponse]],
)
async def get_places_from_collection(
    collection_id: str, requester=Depends(get_current_user(optional=True))
):
    """Lấy danh sách chi tiết địa điểm từ một collection cụ thể."""
    return await collection_service.get_places_from_collection(
        collection_id, requester.get("uid") if requester else None
    )
...
@collection_router.post(
    "/collections/{collection_id}/contributors",
    response_model=ResponseSchema[CollectionResponse],
)
async def add_contributors_to_collection(
    collection_id: str,
    contributors_request: AddMultipleContributorsRequest,
    requester=Depends(get_current_user(optional=False)),
):
    """Thêm nhiều cộng tác viên vào một collection cụ thể."""
    return await collection_service.add_contributors_to_collection(
        collection_id, requester.get("uid"), contributors_request.contributor_uids
    )


@collection_router.get(
    "/collections/{collection_id}/contributors",
    response_model=ResponseSchema[list[CollectionContributorResponse]],
)
async def get_contributors_from_collection(
    collection_id: str, requester=Depends(get_current_user(optional=True))
):
    """Lấy danh sách chi tiết cộng tác viên từ một collection cụ thể."""
    return await collection_service.get_contributors_from_collection(
        collection_id, requester.get("uid") if requester else None
    )
...
async def remove_contributors_from_collection(
    collection_id: str,
    contributors_request: RemoveMultipleContributorsRequest,
    requester=Depends(get_current_user(optional=False)),
):
    """Xóa nhiều cộng tác viên khỏi một collection cụ thể."""
    return await collection_service.remove_contributors_from_collection(
        collection_id, requester.get("uid"), contributors_request.contributor_uids
    )
...
async def add_tags_to_collection(
    collection_id: str,
    tags_request: AddMultipleTagsRequest,
    requester=Depends(get_current_user(optional=False)),
):
    """Thêm nhiều tag vào một collection cụ thể."""
    return await collection_service.add_tags_to_collection(
        collection_id, requester.get("uid"), tags_request.tags
    )
...
async def remove_tags_from_collection(
    collection_id: str,
    tags_request: RemoveMultipleTagsRequest,
    requester=Depends(get_current_user(optional=False)),
):
    """Xóa nhiều tag khỏi một collection cụ thể."""
    return await collection_service.remove_tags_from_collection(
        collection_id, requester.get("uid"), tags_request.tags
    )
...
@collection_router.get(
    "/collections/{collection_id}/savers",
    response_model=ResponseSchema[list[CollectionSaverResponse]],
)
async def get_savers_from_collection(
    collection_id: str, requester=Depends(get_current_user(optional=True))
):
    """Lấy danh sách chi tiết người dùng đã lưu một collection cụ thể."""
    return await collection_service.get_savers_from_collection(
        collection_id, requester.get("uid") if requester else None
    )
