...
from schemas.notification_schema import NotificationResponse, NotificationUpdateRequest

notification_router = APIRouter()

@notification_router.patch("/users/me/notifications/{notification_id}", response_model=ResponseSchema[NotificationResponse])
async def update_notification(notification_id: str, update_request: NotificationUpdateRequest, requester=Depends(get_current_user(optional=False))):
...
@notification_router.delete("/users/me/notifications/{notification_id}", response_model=ResponseSchema[bool])
