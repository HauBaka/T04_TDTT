...
async def add_view(request: Request, data: AddViewRequest, background_tasks: BackgroundTasks, current_user=Depends(get_current_user(optional=True))):
    if current_user:
...
async def get_top_views(data: TopViewRequest = Depends(), current_user=Depends(get_current_user(optional=True))):
