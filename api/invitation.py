...
async def update_invitation(invitation_id: str, invitation_request: InvitationUpdateRequest, background_task: BackgroundTasks, requester=Depends(get_current_user(optional=False))):
    """Cập nhật trạng thái của một lời mời cụ thể."""
    return await invitation_service.update_invitation(invitation_id, requester.get("uid"), invitation_request, background_tasks=background_task)
