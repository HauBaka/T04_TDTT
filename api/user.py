...
@user_router.get("/users/{username}", response_model=ResponseSchema[UserPublicResponse | UserPrivateResponse])
...
@user_router.get("/me/my-collections", response_model=ResponseSchema[list[CollectionPrivateResponse]])
...
@user_router.get("/me/contributing-collections", response_model=ResponseSchema[list[CollectionPrivateResponse]])
async def get_contributing_collections(current_user=Depends(get_current_user(optional=False))):
    """Lấy danh sách các collection mà người dùng đang cộng tác."""
    return await user_service.get_contributing_collections(requester_uid=current_user['uid'])

@user_router.get("/me/saved-collections", response_model=ResponseSchema[list[CollectionPublicResponse]])
