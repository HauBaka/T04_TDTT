...
class ConversationRepository(BaseRepository):
    ...
    async def create(self, owner_uid: str, request: ConversationCreateRequest, custom_id: Optional[str] = None) -> ConversationDocument:
        ...
        await self._create(conversation_doc.model_dump(mode="python", exclude_none=False), doc_id=conversation_doc.id)
    ...
    async def get_by_id(self, conversation_id: str) -> ConversationDocument:
        ...
        try:
            ...
            logger.error(f"Error validating conversation data for ID {conversation_id}: {str(e)}")
            raise ValidationError("Failed to validate conversation data")
        
    async def get_by_ids(self, conversation_ids: list[str]) -> list[ConversationDocument]:
        ...
        for doc in conversation_docs:
            try:
                ...
                logger.error(f"Error validating conversation data for ID {doc.get('id')}: {str(e)}")
                continue

        return conversations

    async def update(self, conversation_id: str, update_request: ConversationUpdateRequest) -> ConversationDocument:
    ...
    async def add_members(self, conversation_id: str, member_uids: list[str], roles: list[ConversationRole] | None = None) -> ConversationDocument:
        ...
        for i, uid in enumerate(member_uids):
            ...
            member = ConversationMemberDocument(
                uid=uid,
                role = ConversationRole(roles[i]) if roles and i < len(roles) else ConversationRole.MEMBER,
                joined_at=now
            )
    ...
    async def remove_members(self, conversation_id: str, member_uids: list[str]) -> ConversationDocument:
    ...
    async def get_recent_messages(self, conversation_id: str, limit: int = 20) -> list[ConversationMessageDocument]:
        ...
        for msg in messages:
            try:
                message_list.append(ConversationMessageDocument.model_validate(msg.to_dict()))
            except PydanticValidationError as e:
                logger.error(f"Error validating message data for ID {msg.id} in conversation {conversation_id}: {str(e)}")
                continue

        return message_list

    async def get_message_by_id(self, conversation_id: str, message_id: str) -> ConversationMessageDocument:
        """
        ...
        """
        doc = await self._db.collection("conversations").document(conversation_id).collection("messages").document(message_id).get()
        ...
        try:
            ...
            logger.error(f"Error validating message data for ID {message_id} in conversation {conversation_id}: {str(e)}")
            raise ValidationError("Failed to validate message data for retrieval")
    
    async def get_member(self, conversation_id: str, member_id: str) -> ConversationMemberDocument:
        """Lấy thông tin một thành viên cụ thể trong một conversation."""
        doc = await self._db.collection("conversations").document(conversation_id).collection("members").document(member_id).get()
        ...
        try:
            ...
            logger.error(f"Error validating member data for UID {member_id} in conversation {conversation_id}: {str(e)}")
            raise ValidationError("Failed to validate member data for retrieval")

    async def get_members(self, conversation_id: str) -> list[ConversationMemberDocument]:
        ...
        for member in members:
            try:
                member_list.append(ConversationMemberDocument.model_validate(member.to_dict()))
            except PydanticValidationError as e:
                logger.error(f"Error validating member data for conversation {conversation_id}: {str(e)}")
                continue

        return member_list

    async def get_detailed_members(self, conversation_id: str) -> list[ConversationMemberResponse]:
        ...
        for member in members:
            ...
            if not user_info:
                logger.warning(f"User info not found for UID {member.uid} when building conversation member response")
                continue

            response_members.append(ConversationMemberResponse(
                uid=member.uid,
...
# --- CÁC HÀM XỬ LÝ SUB-COLLECTION CỦA USER --- (có thể sử dụng đến)
    ...
    async def upsert_user_conversation_summary(self, uid: str, conversation_id: str, update_req: UserConversationSummaryUpdate):
