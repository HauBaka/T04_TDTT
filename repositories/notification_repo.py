...
class NotificationRepository(BaseRepository):
    ...
    async def get_by_id(self, notification_id: str) -> NotificationDocument:
        ...
        try:
            ...
            logger.error(
                f"Error validating notification data for {notification_id}: {str(e)}"
            )
            raise ValidationError("Invalid notification data")

    async def update(
        self, notification_id: str, update_request: NotificationUpdateRequest
    ) -> NotificationDocument:
