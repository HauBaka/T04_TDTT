...
class TripRepository(BaseRepository):
    ...
    async def create(self, uid: str, trip_data: TripCreateRequest) -> TripDocument:
        ...
        await self._create(
            trip_doc.model_dump(mode="python", exclude_none=False), trip_doc.id
        )
        await self.add_members(
            trip_doc.id, [uid]
        )  # Tự động thêm creator vào member list
        return trip_doc
    ...
    async def update(
        self, trip_id: str, update_data: TripUpdateRequest
    ) -> TripDocument:
    ...
    async def add_members(self, trip_id: str, members_data: list[str]) -> TripDocument:
        ...
        batch.update(
            trip_ref,
            {"updated_at": now, "member_uids": firestore.ArrayUnion(members_data)},
        )
    ...
    async def remove_members(self, trip_id: str, uids: list[str]) -> TripDocument:
        ...
        for uid in uids:
            ...
            # Xóa cache của member đó
            member_keys.append(cache_key("users", "id", uid))

        await self._commit_batch(batch)
        # Xóa cache trip và user liên quan
        await cache_delete(self._build_cache_key("id", trip_id), *member_keys)

        return await self.get_by_id(trip_id)

    async def update_members(
        self, trip_id: str, updates_data: dict[str, dict]
    ) -> list[TripMemberDocument]:
            ...
            if doc.exists:
                ...
                try:
                    updated_members.append(
                        TripMemberDocument.model_validate(member_data)
                    )
    ...
    async def delete(self, trip_id: str) -> bool:
        ...
        member_uids = trip.member_uids if trip else []

        trip_ref = self._collection.document(trip_id)

        await self._delete_subcollection(trip_ref.collection("members"))

        await self._delete(trip_id)

        member_keys = [cache_key("users", "id", uid) for uid in member_uids]
