...
class BehaviorEventRepo(BaseRepository):
    ...
    async def list_events_for_user(
        self, user_id: str, limit: int = 100, last_doc = None
        ...
        for doc in docs:
            ...
            if data:
                data["id"] = doc.id
                events.append(UserBehaviorEvent.model_validate(data))

        return events

    async def get_event_by_id(self, event_id: str) -> UserBehaviorEvent | None:
        """Lấy một event cụ thể theo ID."""
        data = await self._get_by_id(event_id)
        if data:
            return UserBehaviorEvent.model_validate(data)
        return None
    ...
    async def count_events_for_user(self, user_id: str) -> int:
        ...
        try:
            ...
            logger.warning("Unexpected aggregation response for user_id=%s", user_id)
            return 0
