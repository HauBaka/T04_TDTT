...
class InvitationRepository(BaseRepository):
    ...
    async def create(self, invitation_request: InvitationCreateRequest) -> InvitationDocument:
        """Tạo một lời mời mới."""
    ...
    async def create_batch(self, invitation_requests: list[InvitationCreateRequest]) -> list[InvitationDocument]:
        ...
        for req in invitation_requests:
            ...
            batch.create(doc_ref, invitation_doc.model_dump(mode="python", exclude_none=False))
            invitations.append(invitation_doc)

        await self._commit_batch(batch)
        return invitations

    async def get_pending_invitations_for_user(self, requests: list[GetPendingInvitationsRequest]) -> dict[str, InvitationDocument]:
        ...
        for i in range(0, len(target_uids), CHUNK_SIZE):
            ...
            query = (
                self._collection
                .where(filter=FieldFilter("target_uid", "in", chunk))
            ...
            try:
                ...
                logger.error(f"Error querying pending invitations chunk {chunk}: {str(e)}")
                continue
            ...
            for doc in docs:
                ...
                try:
                    ...
                    logger.error(f"Error validating invitation data for document {doc.id}: {str(e)}")
                    continue
