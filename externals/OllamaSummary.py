...
class LocalOllamaClient:
    ...
    def generate_content(self, prompt: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Local AI Error (Status {e.response.status_code}): {e.response.text}"
            )
            return self._fallback_generate(prompt)
    ...
    def _fallback_generate(self, prompt: str) -> str:
        try:
            ...
            nearby_section = self._extract_section(
                prompt, "[3. ĐỊA ĐIỂM LÂN CẬN (Khoảng cách)]"
            )
            weather_section = self._extract_section(
                prompt, "[4. DỰ BÁO THỜI TIẾT TẠI ĐIỂM ĐẾN]"
            )
            ...
            result = {"overview": overview, "pros": pros, "cons": cons, "notes": notes}
    ...
    def _analyze_reviews(self, reviews_text: str) -> tuple[list[str], list[str], str]:
        ...
        notes = (
            sentences[0][:100] if sentences else "Khách hàng có trải nghiệm khá tốt."
        )
        return pros, cons, notes

    def _generate_overview(
        self, hotel_name: str, reviews: str, amenities: str, nearby: str, weather: str
    ) -> str:
        parts = [f"{hotel_name}"]
        if "Chưa có" not in reviews:
            parts.append("có đánh giá tích cực")
        if amenities and "Không có" not in amenities:
            a_list = [a.strip() for a in amenities.split(",") if a.strip()][:2]
            if a_list:
                parts.append(f"có {', '.join(a_list)}")

        weather_note = ""
        if "mưa" in weather.lower():
            weather_note = " Dự báo có mưa, nên lưu ý lịch trình."
        elif "nắng" in weather.lower() or "nóng" in weather.lower():
            weather_note = " Thời tiết nắng đẹp."
