...
class SerpAPIClient:
    ...
    async def get_status(self) -> dict:
        ...
        try:
            response = await client.get(
                self.account_url, params={"api_key": self.api_key}
            )
            ...
            return {
                "status": "error: 401 - Invalid API Key"
                if response.status_code == 401
                else f"error: HTTP {response.status_code} - {response.text}"
            }
    ...
    def get_place_details(self, property_token: str) -> dict:
        return {
            "details": f"This is a mock details result for property_token {property_token} from SerpAPI."
        }
    ...
    async def search_places(
        ...
        client = get_http_client()
        try:
            response = await client.get(self.hotel_search_url, params=parameters)
                ...
                try:
                    ...
                    if "gps_coordinates" in hotel:
                        gps_coordinates = GPSCoordinates(
                            ...
                            geohash=pgh.encode(
                                hotel["gps_coordinates"].get("latitude", 0.0),
                    ...
                    if "images" in hotel:
                        for obj in hotel["images"]:
                            if not obj.get("thumbnail") or not obj.get(
                                "original_image"
                            ):
                                continue
                    ...
                    hotel_obj = DiscoverHotel(
                        ...
                        price=(hotel.get("rate_per_night") or {}).get(
                            "extracted_lowest", 0
                        ),
                        deal=hotel.get("deal"),
                        images=images,
                    ...
                    logger.warning(
                        f"Error parsing hotel data: {str(e)} - Data: {hotel}"
                    )
                    continue
            return SerpAPIResultSchema(
                status_code=200,
                data=parsed_hotels,
                next_page_token=data.get("serpapi_pagination", {}).get(
                    "next_page_token"
                ),
            )
