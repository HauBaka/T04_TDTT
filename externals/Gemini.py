    ...
    def get_status(self) -> dict:
        try:
            ...
            return {
                ...
                "content": "connected"
                if model_info and hasattr(model_info, "name")
                else "not available",
            }
        except Exception as e:
            logger.error(
                beauty_json(
                    {"Error checking Gemini API status": getattr(e, "message", str(e))}
                )
            )
            return {"model_name": self.default_model, "content": "Not available"}
