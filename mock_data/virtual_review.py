...
class VirtualReview:
    ...
    def initialize(self, source_path: str):
        ...
        if not self.source_path.exists():
            raise FileNotFoundError(
                f"Virtual review source file not found: {self.source_path}"
            )

        df = pd.read_csv(self.source_path)
        self.virtual_reviews = [
            UserReview(text=row["Review"], raw_stars=float(row["Rating"]))
            for _, row in df.iterrows()
        ]

        logger.info(
            f"Imported {len(self.virtual_reviews)} virtual reviews from {self.source_path}"
        )
    ...
    def add_random_reviews(self, place: DiscoverHotel, min_count: int, max_count: int):
        ...
        if min_count > max_count:
            logger.warning(
                f"Invalid count range: min_count ({min_count}) > max_count ({max_count}). Adjusting to min_count = max_count."
            )
            min_count = max_count
        ...
        if actual_count > 0:
            ...
            place.raw_rating = sum(r.raw_stars for r in place.user_reviews) / len(
                place.user_reviews
            )
