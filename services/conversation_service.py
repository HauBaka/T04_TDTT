...
class ConversationService:
    ...
    async def create_conversation(self, owner_uid: str, request: ConversationCreateRequest) -> ResponseSchema[ConversationResponse]:
    ...
    async def update_conversation(self, conversation_id: str, requester_uid: str, update_req: ConversationUpdateRequest, background_tasks: BackgroundTasks) -> ResponseSchema[ConversationResponse]:
        ...
        updated_res = await self.conversation_repository.update(conversation_id, update_req)

        # Đồng bộ tên/ảnh mới cho tóm tắt hội thoại của tất cả thành viên
        user_summary_update = UserConversationSummaryUpdate.model_validate(update_req.model_dump(mode="python",exclude_none=True))
        for uid in conv.member_uids: # chạy ngầm task này
            background_tasks.add_task(
                self.conversation_repository.upsert_user_conversation_summary, 
                uid, conversation_id, user_summary_update
            )
    ...
    async def get_members_from_conversation(self, conversation_id: str, requester_uid: str) -> ResponseSchema[list[ConversationMemberResponse]]:
        ...
        return ResponseSchema(data=await self.conversation_repository.get_detailed_members(conversation_id))
    ...
    async def remove_members_from_conversation(self, conversation_id: str, requester_uid: str, target_uids: list[str]) -> ResponseSchema[ConversationResponse]:
        ...
        if not valid_target_uids:
            raise BadRequestError(message="None of the provided UIDs are members of the conversation")
    ...
    async def send_message_to_conversation(self, conversation_id: str, requester_uid: str, message_data: SendMessageRequest, background_tasks: BackgroundTasks) -> ResponseSchema[ConversationMessageResponse]:
        ...
        saved_msg = await self.conversation_repository.send_message(conversation_id, requester_uid, message_data)

        # Vòng lặp cập nhật tóm tắt hội thoại cho TẤT CẢ mọi người trong nhóm
        for uid in conv.member_uids: # chạy ngầm task này
            summary_update = UserConversationSummaryUpdate(
        ...
        return ResponseSchema(data=ConversationMessageResponse(
            id=saved_msg.id,
    ...
    async def delete_message_from_conversation(self, conversation_id: str, message_id: str, requester_uid: str) -> ResponseSchema[bool]:
    ...
    async def get_or_create_default_chatbot_conversation(self, uid: str) -> ResponseSchema[ConversationResponse]:
        ...
        try:
            ...
            conv = await self.conversation_repository.create(uid, new_conv, custom_id=chatbot_conv_id)
    ...
    async def get_recent_messages(self, conversation_id: str, limit: int = 20) -> ResponseSchema[list[ConversationMessageDocument]]:
        """Lấy danh sách tin nhắn gần đây nhất của một conversation."""
        conv = await self.conversation_repository.get_by_id(conversation_id)
        
        messages = await self.conversation_repository.get_recent_messages(conv.id, limit)
    ...
    async def mark_conversation_as_read(self, conversation_id: str, requester_uid: str) -> ResponseSchema[bool]:
        ...
        await self.conversation_repository.reset_user_unread_count(requester_uid, conv.id)
        return ResponseSchema(data=True)
    
    async def _get_members_list(self, conversation_id: str) -> list[ConversationMemberDocument]:
    ...
    async def _build_response(self, conv_doc: ConversationDocument) -> ConversationResponse:
