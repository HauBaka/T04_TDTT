...
class TripService:
    ...
    async def create_trip(
        self, creator_uid: str, trip_data: TripCreateRequest
    ) -> ResponseSchema[TripResponse]:
        """Tạo một trip mới."""
    ...
    async def get_trip(
        self, trip_id: str, requester_uid: str | None
    ) -> ResponseSchema[TripResponse]:
    ...
    async def update_trip(
        self, trip_id: str, requester_uid: str, update_data: TripUpdateRequest
    ) -> ResponseSchema[TripResponse]:
        trip = await self.trip_repo.get_by_id(trip_id)
        ...
        metadata_fields = {
            ...
            "start_at",
            "end_at",
        }  # Metadata chỉ được sửa khi WAITING
        ...
        if "place_id" in update_payload and not await hotel_repo.valid_ids(
            [update_payload["place_id"]]
        ):
            raise BadRequestError(message="Invalid place_id.")
        ...
        if new_status in (TripStatus.ENDED.value, TripStatus.ENDED):
            ...
            if member_uids:
                await self.user_repo.batch_update_users(
                    member_uids, [{"current_trip": None} for _ in member_uids]
                )

        return await self._build_trip_response(updated_trip)

    async def delete_trip(
        self, trip_id: str, requester_uid: str
    ) -> ResponseSchema[bool]:
        """Xóa một trip."""
        trip = await self.trip_repo.get_by_id(trip_id)
        ...
        await self.user_repo.batch_update_users(
            trip.member_uids, [{"current_trip": None} for _ in trip.member_uids]
        )
    ...
    async def send_invitations(
        self, trip_id: str, requester_uid: str, target_uids: list[str]
        ...
        if trip.status != TripStatus.WAITING:
            raise BadRequestError(
                message="Cannot add members. Trip is currently in '{trip.status.value}' status, expected 'waiting'."
            )
        ...
        for uid in new_member_uids:
            ...
            if user_info.current_trip:
                raise BadRequestError(
                    message=f"User {user_info.display_name} is already in a trip."
                )
    ...
    async def add_members_to_trip(
        self, trip_id: str, requester_uid: str, member_uids: list[str]
    ) -> ResponseSchema[TripResponse]:
        """Thêm nhiều thành viên vào một trip."""
        await self.send_invitations(
            trip_id=trip_id, requester_uid=requester_uid, target_uids=member_uids
        )
        trip = await self.trip_repo.get_by_id(trip_id)
    ...
    async def add_accepted_member(
        self, trip_id: str, member_uid: str
    ) -> ResponseSchema[TripResponse]:
        ...
        if user_info.current_trip:
            raise BadRequestError(
                message=f"User {user_info.display_name} is already in another trip."
            )
    ...
    async def remove_members_from_trip(
        self, trip_id: str, requester_uid: str, target_uids: list[str]
    ) -> ResponseSchema[TripResponse]:
        ...
        for uid in target_uids:
            if uid not in trip.member_uids:
                raise BadRequestError(
                    message=f"User {uid} is not a member of this trip."
                )
    ...
    async def get_trip_members(
        self, trip_id: str, requester_uid: str
    ) -> ResponseSchema[list[TripMemberResponse]]:
        ...
        if requester_uid not in member_uids:
            raise PermissionDeniedError(
                message="You do not have permission to view members of this trip."
            )

        members_detail = await self.user_repo.get_users(
            [m.uid for m in trip_members_info]
        )
        ...
        for member in trip_members_info:
            ...
            if not user_info:
                logger.warning(
                    f"User info not found for member UID {member.uid} in trip {trip_id}"
                )
                continue
    ...
    async def _build_trip_response(
        self, trip_data: TripDocument
    ) -> ResponseSchema[TripResponse]:
        ...
        trip_owner_doc = await user_repo.get_user(trip_data.owner_uid)
        trip_owner_response = TripOwnerResponse(
            uid=trip_owner_doc.uid,
            username=trip_owner_doc.username,
            display_name=trip_owner_doc.display_name,
            avatar_url=trip_owner_doc.avatar_url,
        )
        return ResponseSchema(
            data=TripResponse(
                id=trip_data.id,
                owner=trip_owner_response,
                name=trip_data.name,
