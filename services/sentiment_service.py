...
class SentimentService:
    ...
    async def calculate_real_rating(
        self, raw_reviews: list[UserReview]
    ) -> tuple[float, float, list[AnalyzedReview]]:
        """
        ...
        final_real_rating = (
            total_weighted_stars / total_weight if total_weight > 0 else 0.0
        )
    ...
    async def process_places_real_rating(self, filtered_places: list[DiscoverHotel]):
        ...
        for place in filtered_places:
            ...
            expiration_date = (
                sentiment_meta.ai_score_expiration_date if sentiment_meta else None
            )
            has_cache = bool(
                sentiment_meta and sentiment_meta.analyzed_reviews
            )  # Đã có AI review chưa
            ...
            if not raw_reviews or len(raw_reviews) == 0:
                ...
                place.ai_sentiment.ai_score_expiration_date = now + timedelta(
                    days=REAL_RATING_CACHE_EXPIRATION_DAYS
                )
                place.ai_sentiment.trust_weight = (
                    0.0  # Không có review nào -> không đáng tin cậy
                )
        ...
        if sentiment_tasks:
            ...
            new_expiration_date = now + timedelta(
                days=REAL_RATING_CACHE_EXPIRATION_DAYS
            )  # Sử dụng datetime
